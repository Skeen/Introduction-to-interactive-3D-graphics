...
*/
    ...
    function initialize_mouse_points()
    {
        var mouse_points = [];
        var mouse_centers = [];
        // Left edge
        ...
        mouse_num_points = mouse_points.length;
        for(var i = 0; i < mouse_num_points; i++)
        {
            mouse_centers.push(vec2(0, 0));
        }
    ...
    // Setup stick man buffers
    initialize_stick_man();
    // MOve stickman into position
    update_stick_man(0.5 + Math.floor(worldHeight/3), Math.floor(worldHeight/3)+10);
    // Setup mouse outline buffers
    initialize_mouse_points();
    ...
    canvas.addEventListener("mousemove", function (event)
    {
        ...
        var blockX = Math.floor(mousePoint[0]);
        var blockY = Math.floor(mousePoint[1]);
        mouse_pos = vec2(blockX + 0.5, blockY + 0.5);
    ...
    function render()
    {
        ...
        if(mouse_colors.length != 0)
        {
            gl.uniform2fv(vPosOffset, mouse_pos);
            ...
            gl.drawArrays(gl.LINES, 0, mouse_num_points);
        }

        gl.uniform2fv(vPosOffset, vec2(0,0));
    ...
    function initBuffers()
    {
        ...
        vPosOffset = gl.getUniformLocation(boxShaderProgram, 'vPosOffset');
